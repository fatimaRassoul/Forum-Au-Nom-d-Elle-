...
def download_model_from_drive():
    if not os.path.exists("model_camembert3"):
        print("🔽 Téléchargement du modèle depuis Google Drive...")
        os.makedirs("model_camembert3", exist_ok=True)

        # Remplace cet ID par l’ID réel de ton dossier Google Drive partagé
        folder_id = "17rFC8LCj1Vo0nDQuB5UL5ySORUF5VyA4?usp=sharing"
        gdown.download_folder(
            url=f"https://drive.google.com/drive/folders/{folder_id}",
            output="model_camembert3",
            quiet=False,
            use_cookies=False
        )
